...
const LogIn = () => {
    ...
    const formSubmit = async (e) => {
        ...
        try {
            ...
            // ✅ If login is successful, store user details in context & local storage
            if (response.data) {
                console.log("(Login.js) Response data login: ", response.data);
                
                await myContext.addCurrUser(response.data); // ✅ Store all user details

                myContext.displayPortal(true); // ✅ Show modal after successful login
                
                // ✅ Persist login state in local storage
                localStorage.setItem("loggedInUser", JSON.stringify(response.data));
                localStorage.setItem("isUserLoggedIn", JSON.stringify(true));

                console.log("Stored user in localStorage:", localStorage.getItem("loggedInUser"));
            }
        } catch (error) {
            if (error.response && error.response.status === 401) {
                setWrongPassword(true);
